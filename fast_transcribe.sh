...
# Fast GPU Transcription - Optimized EC2 Startup Script
# ====================================================
# AMI-optimized: All dependencies pre-installed, cached models ready
# Goal: Start transcription within 10 seconds of instance launch
# Focus: Direct model loading from optimized cache
...
# AMI verification - check for optimized cache setup
...
if [[ ! -d "/opt/transcribe/models" ]]; then
    log_msg "ERROR: Model cache directory not found - missing /opt/transcribe/models" "ERROR"
    log_msg "AMI $AMI_ID was not properly built or model cache creation failed" "ERROR"
    exit 1
fi

if [[ ! -f "/opt/transcribe/models/cache_metadata.json" ]]; then
    log_msg "ERROR: Model cache metadata not found - missing /opt/transcribe/models/cache_metadata.json" "ERROR"
    log_msg "AMI $AMI_ID was not properly built or model pre-download failed" "ERROR"
    exit 1
fi

if [[ ! -f "/opt/transcribe/fast_transcribe.py" ]]; then
    log_msg "ERROR: Transcription script not found - missing /opt/transcribe/fast_transcribe.py" "ERROR"
    log_msg "AMI $AMI_ID was not properly built or script upload failed" "ERROR"
    exit 1
fi

log_msg "AMI verified - setup complete with optimized model cache"
...
log_msg "Verifying model cache..." "STATUS"
...
log_msg "Model cache size: $CACHE_SIZE"
log_msg "Model files found: $MODEL_COUNT"

if [[ $MODEL_COUNT -lt 5 ]]; then
    log_msg "WARNING: Fewer than expected model files found" "ERROR"
fi

log_msg "✅ Model cache verified and ready for fast loading" "STATUS"
...
# Add detailed timing and process monitoring
log_msg "=== PROCESS MONITORING ===" "DEBUG"
log_msg "System resources:" "DEBUG"
free -h | head -2
df -h | head -2
uptime

log_msg "Python executable:" "DEBUG"
which python3
python3 --version

log_msg "Starting Python execution at: $(date +'%Y-%m-%d %H:%M:%S.%3N')" "DEBUG"

# Run transcription with detailed error capture and timing
...
if python3 /opt/transcribe/fast_transcribe.py "$STANDARDIZED_FILENAME" 2>&1; then
    ...
    log_msg "Python execution time: ${PYTHON_DURATION}s" "DEBUG"
