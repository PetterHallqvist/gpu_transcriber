...
"""
        ...
        response = ec2.run_instances(
            ...
            InstanceType=os.environ.get('INSTANCE_TYPE', 'g4dn.medium'),
            MinCount=1,
            MaxCount=1,
...
def lambda_handler(event, context):
    ...
    log(f"Event received: {json.dumps(event)}")
    ...
    try:
        ...
        log(f"Expected bucket: {S3_BUCKET}, Expected prefix: {S3_PREFIX}")
        ...
        if bucket_name != S3_BUCKET or not s3_key.startswith(S3_PREFIX):
            ...
            log(f"Bucket match: {bucket_name == S3_BUCKET}, Prefix match: {s3_key.startswith(S3_PREFIX)}")
            return {'statusCode': 200, 'body': json.dumps({'message': 'Ignored'})}
        
        log("Bucket and prefix validation passed")
        
        # Validate file type
        log(f"Checking file extension: {s3_key.lower()}")
        ...
        log("File type validation passed")
